...
menahannya.** Diperbarui 2026-09-17 (sesi kedelapan, komputer Windows terpisah
dari sesi-sesi sebelumnya — K4/K9/K11 dari tabel "Pengumpulan sinyal
terjadwal" dikerjakan tuntas dan sudah live di production; K7/K8 diriset
tuntas dan siap dieksekusi tapi butuh token admin produksi; K10/K2 dianalisis
ulang. Sebelumnya 2026-09-16, sesi ketujuh — K1/K3/K5/K6 dikerjakan; lihat
blok "MULAI DARI SINI" di kepala `docs/deployment-status.md` untuk detail dan
bukti kedua sesi).
...
| K2 | **Data MBG 12–15 Sep hilang permanen -- ditinjau ulang 2026-09-17, kesimpulan TIDAK berubah.** Dicoba cek Wayback Machine (`web.archive.org/cdx/...`) untuk snapshot arsip situs penerbit di jendela tanggal itu -- **layanannya sedang offline/rate-limited saat dicek**, tidak dipaksakan lebih lanjut (bukan alasan utama tetap terbuka, cuma satu jalur yang belum tuntas dicoba). | Deret harian proyek MBG bolong 4 hari; analisis tren harus menyebutnya. | Tidak bisa dipulihkan dari RSS -- sudah pasti. Pemulihan manual dari arsip penerbit **masih blocker ganda** sekalipun IA nanti bisa diakses: (1) belum tentu ketemu artikel yang persis relevan, (2) `POST /signals/ingest` tetap butuh kredensial admin produksi yang agen tidak punya (blocker sama seperti K7/K8). Bukan prioritas untuk dikejar lebih jauh tanpa permintaan eksplisit pengguna -- rasio usaha/manfaatnya rendah dibanding item lain. |
...
| K4 | ✅ **Selesai 2026-09-17.** `.github/workflows/keepalive.yml` baru: commit kosong lewat `GITHUB_TOKEN` bawaan tiap tanggal 1 & 15 (`permissions: contents: write` eksplisit, karena default repo ini "read" -- dicek lewat `GET .../actions/permissions/workflow`). Dikonfirmasi run terjadwal `collect-signals.yml` sendiri TIDAK menghitung sebagai "aktivitas repo" GitHub (aktivitas = push/release/merge, bukan run workflow yang tidak mengubah apa pun). | Pengumpulan bisa berhenti diam-diam sekitar pertengahan November bila repo tidak di-commit. | Selesai -- tanpa PAT baru atau layanan pihak ketiga, beda dengan mitigasi K1 yang butuh keputusan vendor. Kalau dua run keepalive berturut-turut (1 bulan) gagal, tetap aktifkan ulang manual di tab Actions. |
...
| K8 | 🔶 **Diriset 2026-09-17, siap dieksekusi -- butuh token admin produksi yang agen tidak punya (sama blocker seperti K7).** **13 sumber RSS diverifikasi hidup** langsung ke server: 6 nasional yang sudah dipakai MBG (Antara `antaranews.com/rss/terkini.xml`, CNBC Indonesia, Republika, Sindonews, CNN Indonesia `/nasional/rss`, Tempo -- item count-nya cocok persis dengan yang tercatat 2026-09-15) + **7 regional BARU** yang belum pernah diverifikasi sesi mana pun: `{jambi,kalsel,kalbar,kalteng,kaltim,sumsel,riau}.antaranews.com/rss/terkini.xml`, semua 200 dengan 20 item/~7-16 jam (aman dari `coverage_gap`). Liputan6 (dipakai project ini sebelumnya) gagal di 4 URL yang dicoba -- URL persisnya cuma ada di config production, tidak di repo. | Tidak terkumpul otomatis sama sekali -- riset karhutla Kalimantan/Sumatra pengguna butuh data baru terus, bukan snapshot 157 item dari sesi 2026-09-11. | **Siap dieksekusi, urutan:** (1) cari `project_id` KEBAKARAN HUTAN lewat `GET /projects` (tidak tercatat di docs manapun -- proyek riset pengguna, bukan bagian deploy platform); (2) `POST .../signals/sources` 13× dengan `connector: "rss"`, `config: {"feed_url": "<url>", "keywords": "karhutla, kebakaran hutan, kabut asap, titik api, hotspot", "label": "<nama outlet>"}` (keywords ini USULAN, sesuaikan kalau pengguna mau cakupan beda dari studi 2026-09-11 yang manual, tidak pakai filter kata kunci otomatis); (3) `POST .../signals/collector-token`; (4) tambahkan token sebagai **baris baru** (bukan ganti) di secret `POP_COLLECTOR_TOKENS` -- **langkah ini WAJIB persetujuan eksplisit pengguna**, itu secret CI/CD yang dipakai token proyek lain juga. Agen tidak simpan token admin produksi apa pun; browser automation gagal konek Chrome di sesi ini (lihat K7). |
| K9 | ✅ **Selesai 2026-09-17.** 16 tes baru di `apps/api/tests/test_collect_signals_script.py`, server HTTP palsu dari stdlib `http.server` (tanpa dependensi baru) — cakupan `wake()` retry/timeout, `collect()` retry 502/503/504, token bukan `type=collector` ditolak, proyek tanpa sumber, `main()` exit code + `GITHUB_STEP_SUMMARY`. CI PR #20 hijau. | Salah parsing respons baru ketahuan saat run merah. | Selesai — [PR #20](https://github.com/mahendrasenoaji-lgtm/public-opinion-platform/pull/20), merged ke `main`. |
| K10 | 🔵 **Dianalisis 2026-09-17 -- kesimpulan: kemungkinan BUKAN masalah sekarang, tapi nomornya belum dikonfirmasi dashboard (agen tidak punya akses).** Asumsi awal "ping tiap 30 menit" **sudah terbukti keliru oleh K1**: jadwal GitHub nyatanya di-throttle jadi ~5-9 run/hari (bukan 48), dan API tidur setelah 15 menit sepi -- jarak antar run (2,5-5,5 jam) jauh di atas itu, jadi tiap run bayar cold-start penuh (~93 dtk terukur) + durasi `collect-all` (~20-40 dtk). Estimasi: ~7 run/hari x ~2,25 menit terbangun = **~8 jam/bulan**, <2% dari 750 jam gratis -- untuk servis `pop-api` ini SENDIRI. `keepalive.yml` (K4, baru) tidak menambah beban ini sama sekali (cuma `git commit`, tidak memanggil Render). | 750 jam gratis/bulan dibagi semua layanan free di **workspace** Render yang sama -- risiko sebenarnya ada di layanan LAIN di workspace itu, yang agen tidak punya visibilitas ke dashboard Render untuk mengeceknya. Kalau K1 nanti "diperbaiki" jadi jadwal 30 menit sungguhan (cron eksternal), estimasi naik ke ~54 jam/bulan untuk servis ini saja -- masih di bawah kuota sendirian, tapi menumpuk dengan servis lain. | Tidak butuh tindakan segera untuk `pop-api` sendiri berdasarkan jadwal AKTUAL sekarang. Cek dashboard Render sekali (butuh login -- agen tidak punya akses) untuk tahu total pemakaian gabungan workspace, terutama kalau ada layanan free lain di workspace `alwayslearn` yang sama. Revisit kalau K1 diperbaiki ke jadwal 30 menit sungguhan. |
| K11 | ✅ **Selesai 2026-09-17 di mesin Windows ini** (beda fisik dari Mac yang dipakai sesi-sesi sebelumnya, jadi ini setup terpisah, bukan lanjutan). `gh auth status` **sudah** punya scope `workflow` di mesin ini (tidak perlu tindakan). pgvector: Docker Desktop dinyalakan, `docker run -p 5433:5432 pgvector/pgvector:pg16` (port 5433 karena 5432 sudah dipakai container proyek lain di mesin ini), `db/schema.sql` + `db/rls.sql` diterapkan langsung tanpa shim, **593/593 tes lulus** terhadap pgvector asli. Container tes dihapus lagi sesudahnya (konvensi "mesin bersih" sesi-sesi sebelumnya). | Tes lokal butuh schema yang di-shim di mesin yang belum pernah disiapkan; mengubah file workflow butuh editor web kalau `gh` tanpa scope `workflow`. | Selesai untuk mesin ini. Kalau kerja lagi dari komputer lain: `docker compose up -d db` (docker-compose.yml repo sudah pakai image `pgvector/pgvector:pg16` yang benar) — cukup pastikan port 5432 lokal bebas, atau ganti port seperti di sini. |
